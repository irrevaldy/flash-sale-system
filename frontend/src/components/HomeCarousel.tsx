    ...
    <section className="hc-section">
      <div className="hc-section-header">
        ...
        <div className="hc-scroll-btns">
          <button className="hc-scroll-btn" onClick={() => scroll('left')}>‹</button>
          <button className="hc-scroll-btn" onClick={() => scroll('right')}>›</button>
        </div>
      </div>

      <div ref={trackRef} className="hc-track" onWheel={onWheel}>
        {products.map((p) => {
          ...
          return (
            <div key={p._id} className="hc-card">
              ...
              <div className="hc-card-body">
                ...
                <div className="hc-name" title={p.name} onClick={() => onView(p._id)}>
                  {p.name}
                </div>
                ...
                <div className="hc-price-row">
                  ...
                  {p.compareAtPrice && p.compareAtPrice > p.price && (
                    <span className="hc-compare">${Number(p.compareAtPrice).toFixed(2)}</span>
                  )}
                </div>

                <div className="hc-actions">
                  <button className="hc-btn-view" onClick={() => onView(p._id)}>View</button>
                  <button className="hc-btn-add" onClick={() => onAddToCart(p)}>+ Cart</button>
                </div>
              </div>
            </div>
          );
